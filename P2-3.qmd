...
Through this analysis we found the missing and special values within our dataset. While automated tools like DataExplorer are excellent for identifying standard NA values, our manual wrangling revealed that significant portions of the missing data we disguised as empty string, spaces, "n/a" text strings.

Using Tukey's fences allowed us to systematically isolate outliers in numerical features. By combining `dplyr` for calculation and `tidyr` for reshaping the data, we were able to effectively visualize the true extent of the dataset's inconsistencies. This layed the groundwork for the formal data cleaning phase.
